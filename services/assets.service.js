...
const addAsset = async (data, email) => {
...
const updateAsset = async (data, email) => {
    const user = await User.findOne({ "email.email": email.toLowerCase() });
    if (!user) throw new Error("User not found");
...
const deleteAsset = async (data, email) => {
    const user = await User.findOne({ "email.email": email.toLowerCase() });
    if (!user) throw new Error("User not found");

    const asset = await Assets.findOneAndDelete({ _id: data.id, userId: user._id.toString() });
    if (!asset) throw new Error("Asset not found or unauthorized");
