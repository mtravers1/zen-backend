...
const checkInstitutionProductSupport = async (institutionId, product) => {
  try {
    ...
    if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
      console.log(
        `[checkInstitutionProductSupport] Using cached result for ${institutionId}_${product}: ${cached.supported}`
      );
      return cached.supported;
    }

    console.log(
      `[checkInstitutionProductSupport] Checking support for ${institutionId}_${product}`
    );
    ...
    const supported =
      institution.products && institution.products.includes(product);

    console.log(
      `[checkInstitutionProductSupport] Institution ${institution.name} (${institutionId}) supports ${product}: ${supported}`
    );
    console.log(
      `[checkInstitutionProductSupport] Available products:`,
    ...
    console.error(
      `[checkInstitutionProductSupport] Error checking ${institutionId}_${product}:`,
      error
    );
...
const saveAccessToken = async (
  ...
  try {
    ...
    if (existingToken) {
      console.log(
        `[PLAID] Update Mode detected - Updating existing token for itemId: ${itemId}`
      );
      ...
      console.log(
        `[PLAID] Update Mode completed successfully for itemId: ${itemId}`
      );
      return {
        userId,
        accessToken,
    ...
    const accessTokenDoc = new AccessToken({
      ...
      itemId,
      userId, // Use MongoDB ObjectId instead of Firebase UID
      institutionId,
    });
...
const getAccounts = async (email, uid) => {
  try {
    ...
    for (const token of accessTokens) {
      try {
        const accountsResponse = await getAccountsWithAccessToken(
          token.accessToken
        );
...
const getTransactions = async (email, uid) => {
  try {
    ...
    for (const token of accessTokens) {
      try {
        const transactionsResponse = await getTransactionsWithAccessToken(
          token.accessToken
        );
...
const getLoanLiabilitiesWithAccessToken = async (
  accessToken,
  institutionId = null
) => {
  try {
    ...
    if (institutionId) {
      const supportsLiabilities = await checkInstitutionProductSupport(
        institutionId,
        "liabilities"
      );
...
const getAccessTokenFromItemId = async (itemId, uid = null) => {
  try {
    ...
    if (!accessTokenDoc) {
      ...
      console.log(
        `[PLAID] Debug: Checking if AccessToken collection exists...`
      );

      try {
        const totalTokens = await AccessToken.countDocuments();
        console.log(
          `[PLAID] Debug: Total AccessToken documents in collection: ${totalTokens}`
        );
        ...
        if (similarTokens.length > 0) {
          console.log(
            `[PLAID] Debug: Found similar itemIds:`,
            similarTokens.map((t) => ({ itemId: t.itemId, userId: t.userId }))
          );
        }
      } catch (debugError) {
        console.error(
          `[PLAID] Debug: Error checking collection:`,
          debugError.message
        );
      }
    ...
    console.log(`[PLAID] Found AccessToken document:`, {
      ...
      accessTokenLength: accessTokenDoc.accessToken
        ? accessTokenDoc.accessToken.length
        : 0,
    ...
    if (!targetUid) {
      if (accessTokenDoc.userId) {
        ...
        await markTokenAsCorrupted(
          itemId,
          "Missing userId in AccessToken document"
        );
        return null;
      }
    }
    ...
    if (
      accessTokenDoc.userId &&
      accessTokenDoc.userId.toString() !== targetUid
    ) {
      logPlaidOperation("getAccessTokenFromItemId", false, {
        itemId,
      ...
      await markTokenAsCorrupted(
        itemId,
        `UID mismatch: provided ${targetUid}, token has ${accessTokenDoc.userId}`
      );
      return null;
    }
    ...
    if (!accessToken) {
      ...
      await handleCorruptedAccessToken(
        itemId,
        targetUid,
        new Error("Access token decryption failed")
      );
...
const cleanUpCorruptedToken = async (itemId) => {
  try {
    ...
    if (result.deletedCount > 0) {
      console.log(
        `[PLAID] Successfully removed corrupted token for itemId: ${itemId}`
      );

      // Also clean up related PlaidAccount documents
      const PlaidAccount = (await import("../database/models/PlaidAccount.js"))
        .default;
      const accountResult = await PlaidAccount.deleteMany({ itemId });

      if (accountResult.deletedCount > 0) {
        console.log(
          `[PLAID] Also removed ${accountResult.deletedCount} related PlaidAccount documents for itemId: ${itemId}`
        );
      }

      // Clean up related Transaction documents
      const Transaction = (await import("../database/models/Transaction.js"))
        .default;
      const transactionResult = await Transaction.deleteMany({ itemId });

      if (transactionResult.deletedCount > 0) {
        console.log(
          `[PLAID] Also removed ${transactionResult.deletedCount} related Transaction documents for itemId: ${itemId}`
        );
      }
    } else {
    ...
    console.error(
      `[PLAID] Error cleaning up corrupted token for itemId ${itemId}:`,
      error
    );
  }
};
...
const markTokenAsCorrupted = async (itemId, reason) => {
  try {
    console.log(
      `[PLAID] Marking token as corrupted for itemId: ${itemId}, reason: ${reason}`
    );
    ...
    console.log(
      `[PLAID] Successfully marked token as corrupted for itemId: ${itemId}`
    );
  } catch (error) {
    console.error(
      `[PLAID] Error marking token as corrupted for itemId ${itemId}:`,
      error
    );
  }
};
  ...
  try {
    ...
    for (const token of corruptedTokens) {
      try {
        ...
        console.error(
          `[PLAID] Error cleaning up token ${token.itemId}:`,
          error
        );
      }
    }
    ...
    console.log(
      `[PLAID] Successfully cleaned up ${cleanedCount} corrupted tokens`
    );
...
const getAccessTokenDiagnostics = async () => {
  try {
    ...
    const corruptedTokens = await AccessToken.countDocuments({
      status: "corrupted",
    });
    const expiredTokens = await AccessToken.countDocuments({
      status: "expired",
    });
...
const updateChaseTransactions = async (item, accessToken, uid, accounts) => {
  ...
  while (hasMore && retryCount < maxRetries) {
    try {
      ...
      console.log(
        `Chase: ${transactions.length} new, ${modifiedTransactions.length} modified, ${removedTransactions.length} removed`
      );
...
const updateUniversalTransactions = async (
  item,
  accessToken,
  uid,
  accounts
  ...
  const isCapitalOne =
    accounts[0]?.institution_id === "ins_56" ||
  ...
  while (hasMore && iterationCounter < maxIterations) {
    ...
    try {
      console.log(
        `Transaction sync iteration ${iterationCounter}, cursor: ${currentCursor}`
      );
      ...
      console.log(
        `Universal sync: ${transactions.length} new, ${modifiedTransactions.length} modified, ${removedTransactions.length} removed`
      );
      ...
      if (
        ...
        mutationErrorCount++;
        console.log(
          `Data mutation detected (attempt ${mutationErrorCount}/${mutationConfig.maxMutationErrors}), restarting pagination from current cursor`
        );
        ...
        try {
          ...
          console.log(
            `Mutation error for institution: ${
              institutionInfo.data.institution.name
            }${isCapitalOne ? " (Capital One - using extended config)" : ""}`
          );
        } catch (instError) {
        ...
        if (mutationErrorCount >= mutationConfig.maxMutationErrors) {
          console.error(
            `Max mutation errors reached (${mutationConfig.maxMutationErrors}), stopping sync`
          );
          break;
        }
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, mutationConfig.mutationDelay)
        );
  ...
  console.log(
    `Universal transaction sync completed. Processed ${newTransactions.length} new transactions`
  );
  return newTransactions;
};
...
const handleModifiedTransactions = async (
  ...
  try {
    console.log(
      `Processing ${modifiedTransactions.length} modified transactions`
    );
...
const handleRemovedTransactions = async (removedTransactions) => {
  try {
    console.log(
      `Processing ${removedTransactions.length} removed transactions`
    );
...
const updateInvestmentTransactions = async (item) => {
  try {
    ...
    if (institutionId) {
      const supportsInvestments = await checkInstitutionProductSupport(
        institutionId,
        "investments"
      );
...
const updateInvadlidAccessToken = async (item) => {
  try {
    ...
    if (!accessToken) {
      console.error(
        `updateInvadlidAccessToken: Failed to get access token for item: ${item}`
      );
      return "Failed to get access token";
    }
    ...
    if (!accounts || accounts.length === 0) {
      console.warn(
        `updateInvadlidAccessToken: No accounts found for access token: ${accessToken}`
      );
    ...
    console.log(
      `updateInvadlidAccessToken: Marked ${accounts.length} accounts as expired for item: ${item}`
    );
...
const checkIfChaseBank = async (itemId, accessToken) => {
  try {
    ...
    return (
      institutionName.includes("chase") || institutionName.includes("jpmorgan")
    );
  } catch (error) {
...
const retryWithBackoff = async (
  ...
  const config = {
    ...(RATE_LIMIT_CONFIG[institutionType] || RATE_LIMIT_CONFIG.default),
    ...customConfig,
  };
    ...
    try {
      ...
      if (shouldRetry) {
        ...
        console.log(
          `Retry ${i + 1}/${
            config.maxRetries
          } after ${delay}ms for ${institutionType}`
        );
...
const trackPlaidRequest = (
  ...
  if (!success) {
    ...
    if (institution) {
      plaidMetrics.institutionErrors[institution] =
        (plaidMetrics.institutionErrors[institution] || 0) + 1;
    }
  }
...
const getPlaidMetrics = () => {
  const avgResponseTime =
    plaidMetrics.responseTimes.length > 0
      ? plaidMetrics.responseTimes.reduce((a, b) => a + b, 0) /
        plaidMetrics.responseTimes.length
      : 0;
  ...
  return {
    ...
    errorRate:
      plaidMetrics.requests > 0
        ? ((plaidMetrics.errors / plaidMetrics.requests) * 100).toFixed(2)
        : 0,
  };
};
...
const isItemStale = async (itemId, staleThresholdDays = 30) => {
  try {
    ...
    return {
      ...
      lastUpdate: accounts[0]?.updated_at,
    };
  } catch (error) {
...
const recoverStaleTransactions = async (itemId, uid, daysBack = 90) => {
  try {
    ...
    if (!itemStatus.valid) {
      ...
      return {
        success: false,
        error: `Item status invalid: ${itemStatus.status}`,
      };
    }
...
const errorMonitoring = {
  ...
  generateErrorReport: () => {
    ...
    const report = {
      timestamp: now,
      encryptionErrors: Array.from(
        errorMonitoring.encryptionErrors.entries()
      ).map(([key, value]) => ({
        ...value,
        key,
      })),
      ...
      summary: {
        ...
        criticalErrors: Array.from(
          errorMonitoring.encryptionErrors.values()
        ).filter(
          (e) =>
            e.errorCode === "ERR_CRYPTO_INVALID_AUTH_TAG" ||
            e.errorCode === "ERR_CRYPTO_INVALID_IV" ||
            e.error.includes("Unsupported state")
        ).length,
      },
    };
...
const decryptValueWithMonitoring = async (
  cipherTextBase64,
  dek,
  uid,
  context = {}
...
const diagnoseAndFixEncryptionIssues = async (uid) => {
  try {
    ...
    const { checkEncryptionKeyHealth, regenerateUserKeys } = await import(
...
const identifyUsersWithEncryptionIssues = () => {
  try {
    ...
    const issuesList = Array.from(usersWithIssues.values())
      .map((user) => ({
        ...user,
    ...
    return {
      ...
      summary: {
        ...
        totalErrors: Array.from(errorMonitoring.encryptionErrors.values())
          .length,
        criticalUsers: issuesList.filter((u) => u.errorCount > 5).length,
        usersNeedingImmediateAttention: issuesList.filter(
          (u) =>
            u.errorCount > 10 ||
            u.errorTypes.some(
              (t) =>
                t.includes("Unsupported state") || t.includes("authentication")
            )
        ).length,
      },
    };
...
const identifyFailingPlaidItems = async (uid) => {
  try {
    ...
    for (const tokenDoc of accessTokens) {
      try {
        const accessToken = await getAccessTokenFromItemId(
          tokenDoc.itemId,
          uid
        );
    ...
    const summary = {
      ...
      failureRate:
        accessTokens.length > 0
          ? ((failingItems.length / accessTokens.length) * 100).toFixed(1) + "%"
          : "0%",
    };
