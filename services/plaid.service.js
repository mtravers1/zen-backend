    ...
    while (hasMore) {
      ...
      try {
        ...
        for (let transaction of transactions) {
          ...
          const encryptedAmount = transaction.amount != null ? await safeEncrypt(transaction.amount, { transaction_id: transaction.transaction_id, field: "amount" }) : null;
          const encryptedAccountType = accountMap.has(transaction.account_id) && accountMap.get(transaction.account_id).account_type != null ? await safeEncrypt(accountMap.get(transaction.account_id).account_type, { transaction_id: transaction.transaction_id, field: "account_type" }) : null;
        ...
        if (modifiedTransactions.length > 0) {
          ...
          for (const transaction of modifiedTransactions) {
            const encryptedAmount = transaction.amount != null ? await safeEncrypt(transaction.amount, {
