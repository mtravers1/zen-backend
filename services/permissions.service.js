...
const countUserInstitutions = async (userId) => {
  try {
    const institutions = await PlaidAccount.distinct('institution_id', { owner_id: userId });
...
const validateCreateTrips = async (createTripsMax, userId) => {
  ...
  const tripsThisMonth = await countUserTrips(userId, currentMonth, currentYear);
  ...
  try {
    ...
    switch (action) {
      ...
      case 'create_trip':
        const currentMonth = new Date().getMonth() + 1;
        const currentYear = new Date().getFullYear();
        const currentTrips = await countUserTrips(userId, currentMonth, currentYear);
        const canCreateTrip = await validateCreateTrips(rolePermissions.create_trips_max, userId);
      ...
      case 'upload_file':
        ...
        const canUpload = await validateStorage(rolePermissions.storage_max_gb, userId);
      ...
      case 'create_business':
        const currentBusinesses = await countUserBusinesses(userId);
        const canCreateBusiness = await validateBusinesses(rolePermissions.businesses_max, userId);
...
const getCurrentUsage = async (userId) => {
  try {
    return {
      ...
      trips_this_month: await countUserTrips(userId, new Date().getMonth() + 1, new Date().getFullYear())
    };
  } catch (error) {
