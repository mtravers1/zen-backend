...
export const toolFunctions = (context) => ({
    ...
    try {
      ...
      const formsInfo = {
        ...
        'bank': {
          ...
          forms: {
            'Account Application': 'Personal information, ID verification, initial deposit',
            'Loan Application': 'Income verification, credit check, collateral information',
            'Credit Card Application': 'Income, employment, existing debt information',
            'Mortgage Application': 'Income, assets, liabilities, property information'
          },
          requiredDocuments: [
      ...
      if (formTypeLower.includes('w-2') || questionLower.includes('w-2')) {
        ...
        responseData.message = 'Here is comprehensive information about W-2 forms for tax filing.';
      } else if (formTypeLower.includes('1099') || questionLower.includes('1099')) {
        responseData.form = formsInfo['1099'];
        responseData.message = 'Here is information about various 1099 forms for different types of income.';
      } else if (formTypeLower.includes('bank') || questionLower.includes('bank')) {
        responseData.form = formsInfo['bank'];
        responseData.message = 'Here is information about common banking forms and required documents.';
      } else if (formTypeLower.includes('mortgage') || questionLower.includes('mortgage')) {
        responseData.form = formsInfo['mortgage'];
        responseData.message = 'Here is comprehensive information about mortgage applications and required documentation.';
      } else {
        // General forms help
        responseData.availableForms = Object.keys(formsInfo);
        responseData.message = 'I can help you with various US tax and banking forms. Ask about specific forms like W-2, 1099, bank applications, or mortgage forms.';
      }
  ...
  getFinancialKnowledge: async ({ topic, question }) => {
    try {
      ...
      if (topicLower.includes('deadline') || questionLower.includes('deadline') || questionLower.includes('when')) {
        responseData.knowledge = knowledgeBase['tax_deadlines'];
        responseData.message = 'Here are the key tax filing deadlines for 2024.';
      } else if (topicLower.includes('deduction') || questionLower.includes('deduction') || questionLower.includes('reduce')) {
        responseData.knowledge = knowledgeBase['tax_deductions'];
        responseData.message = 'Here are common ways to reduce your taxable income.';
      } else if (topicLower.includes('investment') || questionLower.includes('investment') || questionLower.includes('invest')) {
        responseData.knowledge = knowledgeBase['investment_basics'];
        responseData.message = 'Here are the basics of getting started with investing.';
      } else if (topicLower.includes('credit') || questionLower.includes('credit') || questionLower.includes('debt')) {
        responseData.knowledge = knowledgeBase['credit_management'];
        responseData.message = 'Here are strategies for building and managing your credit.';
      } else if (topicLower.includes('retirement') || questionLower.includes('retirement') || questionLower.includes('401k')) {
        ...
        responseData.message = 'I can help you with various financial topics including tax deadlines, deductions, investments, credit management, and retirement planning. What specific area would you like to learn about?';
      }
