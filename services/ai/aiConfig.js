...
export const AI_CONFIG = {
  ...
  TOOLS: {
    ...
    REQUIRED_FOR_FINANCIAL: [    // Always use tools for these patterns
      'balance', 'net worth', 'transactions', 'accounts', 
      'how much', 'total', 'income', 'expenses', 'cash flow'
    ],
    NEVER_FOR_GENERAL: [         // Never use tools for these patterns
      'how to', 'what is', 'how can i', 'advice', 'tips',
      'hello', 'hi', 'thank you', 'help'
    ]
  },
...
export function detectQuestionType(prompt) {
  ...
  if (AI_CONFIG.TOOLS.REQUIRED_FOR_FINANCIAL.some(keyword => 
    lowerPrompt.includes(keyword))) {
    return 'financial_data';
  }
  
  // General questions (no tools)
  if (AI_CONFIG.TOOLS.NEVER_FOR_GENERAL.some(keyword => 
    lowerPrompt.includes(keyword))) {
    return 'general';
  }
  
  // Navigation/UI questions
  if (lowerPrompt.includes('screen') || lowerPrompt.includes('where am i') || 
...
// Response formatter
export function formatAIResponse(response, data = null, source = null, error = false) {
  return {
    response: response || AI_CONFIG.RESPONSE.FALLBACK_MESSAGE,
    data: data || null,
    source: source || (data ? AI_CONFIG.RESPONSE.TOOL_SOURCE : AI_CONFIG.RESPONSE.DEFAULT_SOURCE),
    error: error || false,
