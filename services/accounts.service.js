...
const getCachedDek = async (uid) => {
  ...
  if (!Buffer.isBuffer(keyData.dek)) {
    ...
    throw new Error(
      `Invalid DEK format for uid: ${uid} - expected Buffer, got ${typeof keyData.dek}`
    );
  }
...
const clearDecryptionCache = (uid = null) => {
  ...
  console.log(
    `[clearDecryptionCache] Decryption cache cleared for ${
      uid ? `uid: ${uid}` : "all users"
    }`
  );
};
...
const clearAllCaches = (uid = null) => {
  ...
  console.log(
    `[clearAllCaches] All caches cleared for ${
      uid ? `uid: ${uid}` : "all users"
    }`
  );
};
...
const safeDecryptValue = async (value, dek, uid) => {
  ...
  console.log(
    `[safeDecryptValue ${requestId}] Timestamp: ${new Date().toISOString()}`
  );
  console.log(`[safeDecryptValue ${requestId}] Input parameters:`, {
    value: value
      ? `${typeof value} (${value.length || 0} chars)`
      : "null/undefined",
    hasDek: !!dek,
    dekType: typeof dek,
  ...
  if (value === null || value === undefined || value === "") {
    console.log(
      `[safeDecryptValue ${requestId}] ✅ Returning original value (null/undefined/empty):`,
      value
    );
    return value;
  }
  ...
  if (typeof value !== "string") {
    console.warn(
      `[safeDecryptValue ${requestId}] ⚠️ Attempting to decrypt non-string value: ${typeof value} (${
        value === null ? "null" : value === undefined ? "undefined" : "other"
      })`
    );
    return value;
  }
  ...
  if (value.trim() === "") {
    console.warn(
      `[safeDecryptValue ${requestId}] ⚠️ Attempting to decrypt empty string`
    );
    return value;
  }
  ...
  if (!dek) {
    console.error(
      `[safeDecryptValue ${requestId}] ❌ DEK is missing for uid: ${uid}`
    );
  ...
  if (!uid) {
    console.error(
      `[safeDecryptValue ${requestId}] ❌ UID is missing for decryption`
    );
  ...
  try {
    console.log(
      `[safeDecryptValue ${requestId}] 🔍 Attempting decryption for value:`,
      {
    ...
    if (decrypted === null) {
      console.warn(
        `[safeDecryptValue ${requestId}] ⚠️ Decryption returned null, this might indicate data corruption`
      );

      // Log additional context for debugging
      console.log(
        `[safeDecryptValue ${requestId}] 🔍 Decryption failure context:`,
        {
          uid: uid,
          valueLength: value.length,
          valueStart: value.substring(0, 20),
    ...
    if (
      error.message.includes("Max decryption attempts exceeded") ||
      error.message.includes("decryption failed") ||
      error.message.includes("Invalid key")
    ) {
      console.warn(
        `[safeDecryptValue ${requestId}] 🧹 Clearing DEK cache for uid: ${uid} due to decryption error`
      );
      clearDekCache(uid);
...
console.log(
  "🔍 [Storage] Service account base64 length:",
...
console.log(
  "🔍 [Storage] Service account JSON string length:",
  serviceAccountJsonString?.length || 0
);
...
const addAccount = async (accessToken, email, uid) => {
  console.log(
    `[addAccount] Starting account creation for email: ${email}, uid: ${uid}`
  );
  ...
  try {
    ...
    if (!accounts || !Array.isArray(accounts)) {
      console.error(
        `[addAccount] Invalid Plaid response: missing or invalid accounts data`
      );
      throw new Error(
        "Invalid response from Plaid API: missing or invalid accounts data"
      );
    }
    ...
    for (let account of accounts) {
      ...
      if (existingAccount) {
        console.log(
          `[addAccount] Account already exists: ${account.account_id}`
        );
      ...
      console.log(
        `[addAccount] Account saved successfully: ${account.account_id}`
      );
    }

    console.log(
      `[addAccount] Account creation completed. Saved: ${savedAccounts.length}, Existing: ${existingAccounts.length}`
    );
    ...
    const responseExistingAccounts = await Promise.all(
      existingAccounts.map(async (ec) => {
        return {
          id: ec.id,
          name: await safeDecryptValue(ec.account_name, dek, uid),
        };
      })
    );
    ...
    if (accountsResponse.item.products.includes("transactions")) {
      try {
        transactionsResponse =
          await plaidService.getTransactionsWithAccessToken(
            decryptedAccessToken
          );
      } catch (error) {
    ...
    if (
      ...
      Array.isArray(transactionsResponse.added)
    ) {
    ...
    if (
      ...
      Array.isArray(investmentTransactionsResponse.investment_transactions)
    ) {
...
const getAccounts = async (profile, uid) => {
  ...
  console.log(
    `[getAccounts Service ${requestId}] Timestamp: ${new Date().toISOString()}`
  );
  ...
  try {
    ...
    if (typeof uid !== "string") {
      console.error(
        `[getAccounts Service ${requestId}] ❌ UID is not a string:`,
        {
          uid: uid,
          uidType: typeof uid,
          uidValue: uid,
    ...
    if (uid.trim() === "") {
      console.error(
        `[getAccounts Service ${requestId}] ❌ UID is empty string`
      );
      throw new Error("UID cannot be empty");
    }

    console.log(
      `[getAccounts Service ${requestId}] ✅ UID validation passed:`,
      {
        uid: uid,
        uidType: typeof uid,
        uidLength: uid.length,
    ...
    console.log(
      `[getAccounts Service ${requestId}] Plaid IDs to search:`,
      plaidIds
    );
    ...
    console.log(
      `[getAccounts Service ${requestId}] Plaid accounts found in DB:`,
      {
    ...
    for (const plaidAccount of plaidAccountsResponse) {
      ...
      try {
        ...
        const decryptedMask = await safeDecryptValue(
          plaidAccount.mask,
          dek,
          uid
        );
        ...
        console.log(
          `[getAccounts Service ${requestId}] Decryption results for account:`,
          {
            _id: plaidAccount._id,
        ...
        console.error(
          `[getAccounts Service ${requestId}] Decryption error for account:`,
          {
            _id: plaidAccount._id,
    ...
    console.log(
      `[getAccounts Service ${requestId}] Final plaid accounts array:`,
      {
    ...
    console.error(
      `[getAccounts Service ${requestId}] Error in getAccounts:`,
      error
    );
    throw error;
  }
...
const getAllUserAccounts = async (email, uid) => {
  ...
  try {
    console.log(
      `[getAllUserAccounts] Starting for user ${uid} at ${new Date().toISOString()}`
    );
    ...
    if (!user) {
      ...
      console.warn(
        `[getAllUserAccounts] User not found for uid: ${uid} - returning empty result`
      );
      structuredLogger.logSuccess("getAllUserAccounts", {
        uid,
        duration,
    ...
    console.log(
      `[getAllUserAccounts] Found user, plaidAccounts count: ${
    ...
    if (!user.plaidAccounts || user.plaidAccounts.length === 0) {
      const duration = Date.now() - startTime;
      console.log(
        `[getAllUserAccounts] No plaid accounts found for user ${uid}`
      );
      structuredLogger.logSuccess("getAllUserAccounts", {
        uid,
        duration,
    ...
    const dek = await getCachedDek(uid);
    console.log(
      `[getAllUserAccounts] DEK retrieved successfully for user ${uid}`
    );

    // Process all accounts in parallel for better performance
    console.log(
      `[getAllUserAccounts] Processing ${accountsResponse.length} accounts in parallel`
    );
    const accounts = await Promise.all(
      accountsResponse.map(async (plaidAccount, index) => {
        try {
          console.log(
            `[getAllUserAccounts] Processing account ${index + 1}/${
              accountsResponse.length
            }: ${plaidAccount._id}`
          );
          ...
          console.log(
            `[getAllUserAccounts] Account ${index + 1} decrypted successfully`
          );
          ...
          console.error(
            `[getAllUserAccounts] Error decrypting account ${plaidAccount._id}:`,
            error
          );
    ...
    const duration = Date.now() - startTime;
    console.log(
      `[getAllUserAccounts] Completed successfully in ${duration}ms, returning ${accounts.length} accounts`
    );
    structuredLogger.logSuccess("getAllUserAccounts", {
      uid,
      duration,
...
const weeklyCashFlowPlaidAccountSetUpTransactions = async (
  ...
  if (!uid) {
    throw new Error(
      "UID is required for weeklyCashFlowPlaidAccountSetUpTransactions"
    );
  }
  ...
  for (const plaidAccount of plaidAccounts) {
    ...
    for (const transaction of transactionsResponse) {
      const dek = await getCachedDek(uid);
      const decryptedAmount = await safeDecryptValue(
        transaction.amount,
        dek,
        uid
      );
...
const getTransactions = async (
  ...
  for (const plaidAccount of accounts) {
    ...
    for (const transaction of transactionsResponse) {
      ...
      const decryptedPrice = await safeDecryptValue(
        transaction.price,
        dek,
        uid
      );
      ...
      const decryptedSubtype = await safeDecryptValue(
        transaction.subtype,
        dek,
        uid
      );
      const decryptedQuantity = await safeDecryptValue(
  ...
  console.log(
    "[getTransactions] Total transactions collected:",
  ...
  console.log(
    "[getTransactions] Returning all transactions:",
...
const getUserTransactions = async (
  ...
  if (!user) {
    console.warn(
      `[getUserTransactions] User not found for uid: ${uid} - returning empty result`
    );
...
const getProfileTransactions = async (
  ...
  try {
    ...
    console.log("[getProfileTransactions] Profile lookup result:", {
      ...
      comparison: `String(${profile?.id}) === ${profileId} = ${
    ...
    if (!profile) {
      console.warn(
        "[getProfileTransactions] Profile not found - returning empty result instead of error"
      );
      return [];
    }
    ...
    if (!plaidIds || plaidIds.length === 0) {
      console.warn(
        "[getProfileTransactions] No plaid accounts found for profile"
      );
      return [];
    }
...
const getTransactionsByAccount = async (
  ...
  if (!account) {
    console.warn(
      `[getTransactionsByAccount] Account not found: ${accountId} - returning empty result`
    );
    return [];
  }
...
const getAccountDetails = async (accountId, profileId, uid) => {
  ...
  const access_token = await AccessToken.findOne({
    ...
    .lean()
    .exec();
  const decryptAccessToken = await safeDecryptValue(
  ...
  if (Array.isArray(liabilitiesList.aprs)) {
    ...
    for (const aprItem of liabilitiesList.aprs) {
      ...
      for (const key of [
        ...
        if (aprItem[key]) {
          decryptedAprItem[key] = await safeDecryptValue(
            aprItem[key],
            dek,
            uid
          );
...
async function getDecryptedAccount(account, dek, uid) {
  ...
  for (const field of binaryFields) {
    if (account[field]) {
      decryptedAccount[field] = await safeDecryptValue(
        account[field],
        dek,
        uid
      );
...
const generateUploadUrl = async (fileName) => {
  try {
    ...
    console.log(
      "🔍 [generateUploadUrl] Signed URL generated successfully:",
      url
    );
    return url;
  } catch (error) {
...
const getCashFlowsByPlaidAccount = async (plaidAccount, uid) => {
  ...
  const profiles = await businessService.getUserProfiles(
    plaidAccount.owner_email || "default@example.com",
    uid
  );
