...
if [ "$DRY_RUN" = true ]; then
  ...
  node scripts/delete-all-users.js --dry-run
  ...
  node scripts/cleanup-firebase-users.js --dry-run
  ...
  node scripts/delete-all-users.js --confirmed-delete-users

  echo "Step 2: Running cleanup for orphaned database records..."
  node scripts/cleanup-orphaned-data.js --no-dry-run

  echo "Step 3: Running cleanup for orphaned Firebase users..."
  node scripts/cleanup-firebase-users.js --no-dry-run
