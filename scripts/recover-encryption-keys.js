...
async function main() {
  ...
  try {
    ...
    console.log('\nRecovery Results:');
    console.log(`  - Recovered: ${recoveryResult.recovered ? '✅ Yes' : '❌ No'}`);
    console.log(`  - New Version: ${recoveryResult.newVersion || 'N/A'}`);
    console.log(`  - Has Previous Key: ${recoveryResult.hasPreviousKey ? '✅ Yes' : '❌ No'}`);
    console.log(`  - Recommendation: ${recoveryResult.recommendation || 'None'}`);
    ...
    if (recoveryResult.recovered) {
      ...
      console.log(`  - New Health Status: ${newHealth.healthy ? '✅ Healthy' : '❌ Unhealthy'}`);
    } else {
