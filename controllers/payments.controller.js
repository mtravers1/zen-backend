...
const PRIVATE_KEY_PATH = 'controllers/SubscriptionKey_X83VSLB3RC.p8'
const ISSUER_ID = '20773a56-3323-4517-a5ad-226e35e0b218'
const KEY_ID = 'X83VSLB3RC'
const ALG = 'ES256'
const BUNLDE_ID = "com.zentavos.zentavosdev"
const privateKeyPEM = fs.readFileSync(PRIVATE_KEY_PATH, 'utf-8');
const environment = Environment.SANDBOX;

const client = new AppStoreServerAPIClient(privateKeyPEM, KEY_ID, ISSUER_ID, BUNLDE_ID, environment)
...
const updateUserUUID = async (req, res) => {
  try {
    ...
    const response = await paymentService.updateUserUUID(
      data.uuid,
      uid
    );
    res.status(201).json(response);
...
const decodeSignedPayload = async (signedPayload) => {
  ...
  try {
    ...
    const transactionId = JSON.parse(Buffer.from(transactionB64[1], 'base64').toString('utf-8'));
...
const validateSubscription = async (originalTransactionId) => {
  ...
  const signedTransactionInfo = JSON.parse(Buffer.from(splited[1], 'base64').toString('utf-8'));
...
const paymentsController = { verifyReceipts, weebhookAndroid, weebhookApple, updateUserUUID };
