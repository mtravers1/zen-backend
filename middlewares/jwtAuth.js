...
async function jwtAuthentication(req, res, next) {
  ...
  console.log(
    `[JWT AUTH ${requestId}] User Agent: ${req.headers["user-agent"]}`,
  );
  console.log(
    `[JWT AUTH ${requestId}] Has authorization header: ${!!authHeader}`,
  );
  console.log(`[JWT AUTH ${requestId}] Authorization header: ${authHeader}`);

  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    console.log(
      `[JWT AUTH ${requestId}] ❌ No Bearer token found - returning 401`,
    );
  ...
  console.log(
    `[JWT AUTH ${requestId}] Token extracted: ${token ? `${token.substring(0, 20)}...` : "null"}`,
  );
  console.log(
    `[JWT AUTH ${requestId}] Token length: ${token ? token.length : 0}`,
  );
