...
import { CorruptedDataError, DecryptionError, decryptValue, encryptValue, getUserDek } from "../database/encryption.js";
import structuredLogger from "./structuredLogger.js";
import { healUnencryptedField } from "./dataHealing.js";
  ...
  return async function safeDecrypt(value, context = {}) {
    ...
    try {
      ...
      if (error instanceof CorruptedDataError) {
        // Data is malformed (e.g., too short). This is not healable automatically.
        // Log it for manual investigation.
        structuredLogger.logError("corrupted_field_detected", {
      ...
      // If it's not a CorruptedDataError, it's likely a DecryptionError,
      // which we will treat as a potential plaintext healing opportunity.
      if (typeof value === 'string' && value.length > 0) {
        const { model, docId, fieldPath } = context;

        if (model && docId && fieldPath) {
          const safeEncrypt = createSafeEncrypt(uid, dek);
          // Heal in the background
          healUnencryptedField({ model, docId, fieldPath, value, safeEncrypt });
        }
        
        // Return the plaintext value for now
        return value;
      }
      
      // If it's not a string or something else went wrong, log the original error
      structuredLogger.logErrorBlock(error, {
