...
import { DecryptionError, decryptValue, encryptValue, getUserDek } from "../database/encryption.js";
...
export function createSafeDecrypt(uid, dek) {
  return async function safeDecrypt(value, context = {}) {
    ...
    try {
      ...
      // If decryption fails, check if the value is a string. If so,
      // it's likely unencrypted legacy data that needs to be healed.
      if (typeof value === 'string' && value.length > 0) {
        ...
        // Return the original plaintext value for immediate use.
        return value;
      }
      
      // If it's not a string or some other error occurred, log it and return null.
