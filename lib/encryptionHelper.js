...
export function createSafeDecrypt(uid, dek) {
  return async function safeDecrypt(value, context = {}) {
    console.log(`[DECRYPT_TRACE] Attempting to decrypt value for field: ${context.field}`);
    console.log(`[DECRYPT_TRACE] Value: ${value}`);
    try {
      ...
      if (error instanceof DecryptionError) {
        ...
        return value; // Return original value on DecryptionError
      } else {
...
export async function safeDecryptNumericValue(value, safeDecrypt, context) {
  ...
  if (typeof decryptedValue === "string") {
    // Assuming that if it's still a string, decryption failed.
    return null;
  }
