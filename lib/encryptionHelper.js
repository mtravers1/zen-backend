    ...
    try {
      ...
      try {
        const result = await structuredLogger.withContext('decryptWithCurrentKey', {
          user_id: uid,
          encryption_key_version: currentDek.version,
          has_cipher_text: !!cipherTextBase64,
          ...context
        }, async () => {
          return await decryptValue(cipherTextBase64, currentDek);
        });
        ...
        structuredLogger.logEncryptionOperation('decrypt', true, {
          user_id: uid,
          key_version: currentDek.version,
          durationMs,
          fallback_used: false
        });
        ...
        if (previousDek) {
          try {
            const result = await structuredLogger.withContext('decryptWithPreviousKey', {
              ...
              return await decryptValue(cipherTextBase64, previousDek);
            });
    ...
    try {
      ...
      const health = {
        ...
        current_key: {
          ...
          created_at: currentDek.createdAt,
          is_valid: true
        },
        previous_key: previousDek ? {
          version: previousDek.version,
          created_at: previousDek.createdAt,
          is_valid: true
        } : null,
  ...
  static async rotateKey(uid, context = {}) {
    ...
    try {
      ...
      const result = await structuredLogger.withContext('rotateEncryptionKey', {
        ...
        // This would typically call a key rotation service
