...
class StructuredLogger {
  ...
  /**
   * Log an error message (alias for logErrorBlock, converts message to Error object)
   */
  logError(message, context = {}) {
    this.logErrorBlock(new Error(message), context);
  }
  ...
  logWarning(message, context = {}) {
    const logData = {
      timestamp: new Date().toISOString(),
      operation: context.operation || "warning",
      status: "warning",
      message,
      ...context,
    };
  ...
  logInfo(message, context = {}) {
    const logData = {
      timestamp: new Date().toISOString(),
      operation: context.operation || "info",
      status: "info",
      message,
      ...context,
    };

    console.log(`ℹ️ Info: ${message}`, JSON.stringify(logData, null, 2));
  }
  ...
  async withContext(operation, context, fn) {
    ...
    try {
      ...
      const errorId = this.logErrorBlock(error, {
        ...operationContext,
        durationMs,
      });
